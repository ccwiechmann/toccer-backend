package de.ccw.toccer.backend.docx;
...
public class DocxExporter {
	...
	public DocxExporter(String filepath, Multimap<String, TocEntry> entriesByCategory) {
	...
	public void export() {
		...
		builder.append(getXmlFile("src/main/resources/docxPrefix.xml"));

		final List<String> categories = new ArrayList<>(entriesByCategory.keySet());
		Collections.sort(categories);

		for (final String category : categories) {
			builder.append(getXmlFile("src/main/resources/docxCategory.xml").replace("{category}", category));

			final List<TocEntry> entries = new ArrayList<>(entriesByCategory.get(category));
			Collections.sort(entries);

			for (final TocEntry entry : entries) {
				builder.append(getXmlFile("src/main/resources/docxEntry.xml").replace("{author}", entry.getAuthor())
						.replace("{title}", entry.getTitle()).replace("{page}", entry.getPage())
						.replace("{issue}", entry.getVolume()));
			}
		}

		builder.append(getXmlFile("src/main/resources/docxSuffix.xml"));

		final Map<String, String> env = new HashMap<>();
		env.put("create", "true");
		final Path path = Paths.get("src/main/resources/docxTemplate.zip");
		final URI uri = URI.create("jar:" + path.toUri());
		try (FileSystem fs = FileSystems.newFileSystem(uri, env)) {
			final Path nf = fs.getPath("word/document.xml");
